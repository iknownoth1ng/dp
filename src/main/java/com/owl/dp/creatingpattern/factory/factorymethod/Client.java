...
public class Client {
    public static void main(String[] args) throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        ...
        javaVideo.produce();
        //当需要增加一个产品 FEVideo 时，只需要增加 FEVideo 具体产品类和 FEFactory 具体工厂类即可，不需要修改原有的产品类和工厂类
        FEVideoFactory feVideoFactory = new FEVideoFactory();
        Video feVideo = feVideoFactory.getVideo();
        feVideo.produce();
        //还可以通过反射机制和配置文件配合，连客户端代码都不需要修改
        // 从文件或数据库等外部渠道获取 工厂类名
        String factoryName = "com.owl.dp.creatingpattern.factory.factorymethod.JavaFactory";
        // 通过反射机制获取工厂类
